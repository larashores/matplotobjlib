...
def rows_to_numpy(string_data, sep=None):
    """
...
def pprint_matrix(matrix, column_lables=None, *, title=None, sep="|"):
    """
    ...
    if str(matrix.dtype).startswith("f"):
        string = " {:^20.10}" + (" " + sep + " {:^20.10}") * (cols - 1)
    else:
        string = " {:^20}" + (" " + sep + " {:^20}") * (cols - 1)
...
def pprint_uncertainty_matrix(value_matrix, dvalue_matrix, column_labels=None, *, title=None, sep="|"):
    """
    ...
    pprint_matrix(new, column_labels, title=title, sep=sep)
